...
class Model:
    def __init__(self, params: dict):
        ...
        factors = params['space_market_dist'].sample(size=self.pgi.size)

        self.pgi_factor = Flow(movements=pd.Series(data=factors,
                                                   index=self.pgi.index,
                                                   dtype=float),
                               units=params['units'],
                               name='Space Market Pricing Factor')

        self.pgi = Flow(movements=self.pgi * self.pgi_factor,
                        units=params['units'],
                        name=self.pgi.name)
        ...
        pv_ncf_cumsum = Flow(movements=self.pv_ncf.cumsum(),
        ...
        self.pv_sums.movements = self.pv_sums[:-1]