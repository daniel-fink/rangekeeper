...
A View created from an Assembly contains the Assembly plus exactly its member
entities and relationships. Apply filters as a separate immutable operation:

```python
graph.view(assembly=assembly).filter(...)
```
