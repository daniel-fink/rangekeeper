...
class Span:
    ...
    def shift(self,
        ...
        if bound == 'start':
            return Span(
                name=name,
                start_date=rk.periodicity.date_offset(
    ...
    def from_date_sequence(
        ...
        date_pairs = [(start, rk.periodicity.date_offset(end, rk.periodicity.Type.DAY, -1)) for (start, end) in date_pairs]
    ...
    def from_num_periods_sequence(
        ...
        for duration in cumulative_durations:
            dates.append(rk.periodicity.date_offset(date=start_date, period_type=period_type, num_periods=duration))
