...
class Market(rk.extrapolation.Form):
    # def __init__(
    #         self,
    #         params: dict,
    #         trend: trend.Trend,
    ...
    #         movements=self.space_market.movements / trend.rent_mean,
    ...
    def __init__(
        ...
        # trend_terms = trend.terms(sequence=periodicity.to_range_index(period_index=sequence))
        ...
        self.space_market_price_factors = rk.flux.Flow(
            movements=self.space_market.movements / initial_rent,
            name='Space Market Price Factors')
        """
        This is the "true value" pricing factor for just the space market, 
        not reflecting the asset market cycle (cap rate). We have actually 
        ...
        self.noise = rk.flux.Flow(
            movements=pd.Series(
                data=self.noise_values,
                index=periodicity.to_datestamps(period_index=sequence)),
            name='Noise')
        """
        This is a symmetric PERT distribution. Note that a random realization
        of noise is generated each period, but it is applied to the value
        ...
        implied_cap_rate_data = self.space_market.movements[1:].reset_index(
            drop=True) / self.historical_value.movements[:-1].reset_index(drop=True)
        implied_cap_rate_data = pd.concat([implied_cap_rate_data, pd.Series(np.nan)], axis=0, join='outer')
        self.implied_cap_rate = rk.flux.Flow(
            movements=pd.Series(
                data=implied_cap_rate_data.values,
                index=periodicity.to_datestamps(period_index=sequence)),
            name='Implied Cap Rate')
    ...
    def terms(
        ...
        """
        Returns the multiplicative factors of the simulated market's trend (rental market) at each value in the sequence.
        """
        return self.trend.terms(sequence=sequence)
