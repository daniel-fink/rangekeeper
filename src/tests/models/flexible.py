    ...
    def __init__(self, params: dict):
        ...
        self.operation_span = rk.duration.Span.from_duration(
            ...
            date=rk.duration.offset(
                date=self.acquisition_span.end_date, duration=rk.duration.Type.DAY
            ),
            duration=rk.duration.Type.YEAR,
        ...
        self.projection_span = rk.duration.Span.from_duration(
            name="Projection",
            date=rk.duration.offset(
                date=self.operation_span.end_date, duration=rk.duration.Type.DAY
            ),
            duration=rk.duration.Type.YEAR,
        )
        ...
        self.pgi_factor = rk.flux.Flow(
            movements=pd.Series(
                data=factors, index=self.pgi.movements.index, dtype=float
            ),
            units=params["units"],
        ...
        operation_periods = self.operation_span.to_sequence(
            frequency=params["frequency"]
        )
        ...
        self.sale_values = rk.flux.Flow.from_sequence(
            ...
            data=list(
                self.ncf.sum().movements[: self.operation_span.end_date]
                / self.cap_rates
            ),
            units=params["units"],
        )
