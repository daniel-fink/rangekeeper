...
class EntityRegistry:
    """Graph-bound registration, lookup, and traversal for entities."""
    ...
    def add(self, entity: Entity) -> Entity:
        return self._graph._add_entities((entity,))[0]

    def add_all(self, entities: Iterable[Entity]) -> tuple[Entity, ...]:
        return self._graph._add_entities(entities)

    def __getitem__(self, entity_id: str) -> Entity:
        return self._graph._entity(entity_id)

    def get(self, entity_id: str) -> Entity | None:
        if not isinstance(entity_id, str):
            raise TypeError("entity_id must be a string")
        return self._graph._entities.get(entity_id)

    def all(self) -> tuple[Entity, ...]:
        return tuple(self._graph._entities.values())
    ...
    def predecessors(
        ...
        canonical = self._graph._resolve_entity(entity)
        return self._graph._predecessors(
            canonical.entity_id,
            relationship,
            entity_ids=frozenset(self._graph._entities),
            relationship_ids=frozenset(self._graph._relationships),
        )
    ...
    def successors(
        ...
        canonical = self._graph._resolve_entity(entity)
        return self._graph._successors(
            canonical.entity_id,
            relationship,
            entity_ids=frozenset(self._graph._entities),
            relationship_ids=frozenset(self._graph._relationships),
...
class RelationshipRegistry:
    """Graph-bound registration, creation, and lookup for relationships."""
    ...
    def add(self, relationship: Relationship) -> Relationship:
        return self._graph._add_relationships((relationship,))[0]

    def add_all(
        self,
        relationships: Iterable[Relationship],
    ) -> tuple[Relationship, ...]:
        return self._graph._add_relationships(relationships)
    ...
    def __getitem__(self, relationship_id: str) -> Relationship:
        return self._graph._relationship(relationship_id)
    ...
    def get(self, relationship_id: str) -> Relationship | None:
        ...
        return self._graph._relationships.get(relationship_id)

    def all(self) -> tuple[Relationship, ...]:
        return tuple(self._graph._relationships.values())


class AssemblyRegistry:
    """Graph-bound registration and membership operations for assemblies."""
    ...
    def add(self, assembly: Assembly) -> Assembly:
        return self._graph._add_assembly(assembly)
    ...
    def include(
        ...
        *members: AssemblyMember,
        ...
        return self._graph._change_assembly(assembly, include=members)
    ...
    def entities(self, assembly: Assembly | str) -> tuple[Entity, ...]:
        canonical = self._graph._resolve_assembly(assembly)
    ...
    def containing(
        ...
        if isinstance(member, Entity):
            canonical_entity = self._graph._resolve_entity(member)
            return tuple(
                assembly
        ...
        if isinstance(member, Relationship):
            canonical_relationship = self._graph._resolve_relationship(member)
            return tuple(
                assembly
                for assembly in self.all()
...
class TaxonomyRegistry:
    """Graph-bound registration and lookup for complete taxonomies."""
    ...
    def get(self, code: str) -> Taxonomy | None:
        if not isinstance(code, str):
            raise TypeError("taxonomy code must be a string")
        return self._graph._taxonomies.get(code)

    def all(self) -> tuple[Taxonomy, ...]:
        return tuple(self._graph._taxonomies.values())
